...
function generate (key, build) {
  ...
  suppose('vue', ['init', `${projectDir}`, key], { debug: process.stdout })
    ...
    .when(/plugins/g).respond(build[4])
    .when(/mockjs/g).respond(build[5])
    .when(/Scss/g).respond(build[6])
    .when(/platforms/g).respond(build[7])
    .when(/ESLint/g).respond(build[8])
    .when(/config/g).respond(build[9])
